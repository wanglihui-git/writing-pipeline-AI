...
WELCOME_CARD_ID = 'AAqKNJTPhojSg'
# ALERT_CARD_ID = os.getenv("ALERT_CARD_ID")
ALERT_CARD_ID = 'AAqKNJTTQRmpP'
# ALERT_RESOLVED_CARD_ID = os.getenv("ALERT_RESOLVED_CARD_ID")
ALERT_RESOLVED_CARD_ID = 'AAqKNJTbDeo5b'
...
def do_p2_im_message_receive_v1(data: P2ImMessageReceiveV1) -> None:
    ...
    print(data.event.message.content)  # 用户发送的消息内容，格式为JSON字符串，包含消息文本和消息中携带的其他信息，如图片、文件等。你可以根据需要解析这个字段来获取用户发送的具体内容。
...
client = lark.Client.builder().app_id('cli_a94e47b263fb5bde').app_secret('1KiZJzGgJcaKbo41PiUoyrrjGefhoW5k').build()
wsClient = lark.ws.Client(
    'cli_a94e47b263fb5bde',
    '1KiZJzGgJcaKbo41PiUoyrrjGefhoW5k',
...
def on_bot_menu(self, data: P2ApplicationBotMenuV6) -> None:
    ...
    try:
        open_id = data.event.operator.operator_id.open_id
        event_key = data.event.event_key
        print(f"收到菜单事件 - open_id: {open_id}, event_key: {event_key}")
        if event_key == "help":
            # feishu_client.send_text(open_id, self._help_text())
            print(f"已回复帮助文本给用户 {open_id}")
        else:
            print(f"未处理的菜单 event_key: {event_key}")
    except Exception as e:
        print(f"菜单事件处理异常: {str(e)}", exc_info=True)
...
def main():
    ...
    cli = lark.ws.Client("cli_a94a59fad4a8dbb3", "HgiWn1i8rlMr9eHzuc2WSM34eyEzR2Fs",
